...
# ── TCP_NODELAY patch ──────────────────────────────────────────────
# uvicorn (h11 protocol) 默认不在连接上设置 TCP_NODELAY，
# 导致 TCP Nagle 算法把 SSE 小包攒起来再发，流式输出变批量。
# monkey-patch H11Protocol.connection_made，每个新连接都设 TCP_NODELAY。
try:
    from uvicorn.protocols.http.h11_impl import H11Protocol as _H11

    _orig_connection_made = _H11.connection_made

    def _connection_made_with_tcp_nodelay(self, transport):
    ...
    _H11.connection_made = _connection_made_with_tcp_nodelay
    log.info("miao.tcp_nodelay_patched")
except Exception:
    pass
