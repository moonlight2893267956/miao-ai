...
| **libpq** | 本地跑 `pg_dump` / `psql`（迁移 DB 时用） | `brew install libpq && export PATH="/opt/homebrew/opt/libpq/bin:$PATH"` |
...
**强制流程**：本地 dev **必须**用 `.env.local`，不能直接 `cp .env.example .env` 然后填生产凭证跑本地 —— 会污染生产 Langfuse trace 和 Neon DB。

### 2.1 在两个云服务建 dev 资源

一次性操作，不重复：

1. **Neon**：登录 [console.neon.tech](https://console.neon.tech) → 选 miao-ai 项目 → **Branches** → **Create Branch** 命名 `dev` → 等几秒 → **Connection Details** 选 `Connection string` 复制
...
> 详细步骤：供应商侧看 [`infra/CLOUD_SETUP.md`](../../infra/CLOUD_SETUP.md)。Neon/Langfuse/腾讯云 COS/阿里云 DashScope 都要注册，但本地 dev **只需要** Neon + Langfuse 两个。
...
#   DATABASE_URL=postgresql+asyncpg://...@ep-xxx-pooler.../neondb?ssl=require
...
> **DATABASE_URL 协议前缀必须是 `postgresql+asyncpg://`，query 必须是 `?ssl=require`** —— asyncpg 不认 `postgresql://` 或 `?sslmode=require`（详情见 [`docs/operations/troubleshooting.md`](troubleshooting.md)）
...
```
▶ 加载本地凭证 .env.local（隔离生产 Langfuse/Neon）
✅ 启动完成
...
| 启动后 `/api/v1/health/ready` 报 `psycopg2` ImportError | `.env.local` 的 `DATABASE_URL` 用了 `postgresql://`（非 `postgresql+asyncpg://`） |
| 启动后 `/api/v1/health/ready` 500 报 `users does not exist` | Neon dev branch `search_path` 默认为空，确认 `backend/app/db.py` 包含 connect event listener（git 已修）。若新装项目跑过 `pip install -e .` 也要重新 `pip install -e .` 重装 |
...
| 登录 401 / invalid credentials | 密码是数据库 `users.password` 字段的明文。看下 `psql` 实际值。本地 dev 用 prod 同步过来的数据时，密码是 prod 的真实密码（如 `WXY...`）而不是 `12345678` |
